						...
						<div class="card-body">
							...
							<div class="row">
								<div class="col-xs-12 d-flex justify-content-between align-items-center">
									...
									<a href="#" ng-click="check_addRecord_access(user_id, token)" class="btn btn-info" style="color: white;">Create Kanban Details Done</a>
								</div>
							</div>
							<br/>
        ...
        <script>
            ...
            app.controller("myCtrl", function($scope, $http, $timeout) {
				...
				$scope.check_addRecord_access = function(user_id, token) {
                    $http.get("<?= base_url('admin_add_kanban_details_done'); ?>/" + user_id + "/" + token)
                    .then(function(response) {
                        if (response.data.error) {
                            alert(response.data.error); // Show alert for unauthorized access
                            window.history.back(); // Go back to the previous page
                        } else {
                            window.location.href = "<?= base_url('admin_add_kanban_details_done'); ?>/" + user_id + "/" + token;
                        }
                    })
                    .catch(function(error) {
                        console.error("Error:", error);
                ...
                $scope.datatable = $('#basic-datatable').DataTable({
					...
					"order": [],
					"ajax": {
						"url": '<?= base_url('api/getKanbanDetailsDoneList') ?>',
						"type": 'POST',
				...
				$scope.chosenData = function(data) {
					...
					location.href = "<?= base_url('admin_edit_kanban_details_done') ?>/" + $scope.user_id + "/" + $scope.token + "/" + data;
				}

				$scope.deleteData = function(id) {
					var c = confirm("Are You Sure Want To Delete This DOne Task?");
					if (c) {
						var tobeSubmit = {};
						...
						$http.post("<?= base_url('api/deleteKanbanDetailsDone') ?>", tobeSubmit).then(function(response) {
