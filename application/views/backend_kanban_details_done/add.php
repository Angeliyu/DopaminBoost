            ...
            app.controller("myCtrl", function($scope, $http, $timeout) {
                ...
                $scope.delete = function() {
                    ...
                    $http.post("<?= base_url('api/deleteKanbanDetailsDone') ?>", tobeSubmit).then(function(response) {
                        ...
                        if (response.data.status == "OK") {
                            location.href = "<?= base_url('admin_kanban_details_done'); ?>/" + $scope.user_id + "/" + $scope.token;
                        } else {
                ...
                $scope.backList = function() {
                    $http.get("<?= base_url('admin_kanban_details_done'); ?>/" + $scope.user_id + "/" + $scope.token)
                    .then(function(response) {
                        if (response.data.error) {
                            alert(response.data.error); // Show alert for unauthorized access
                            window.history.back(); // Go back to the previous page
                        } else {
                            window.location.href = "<?= base_url('admin_kanban_details_done'); ?>/" + $scope.user_id + "/" + $scope.token;
                        }
                    })
